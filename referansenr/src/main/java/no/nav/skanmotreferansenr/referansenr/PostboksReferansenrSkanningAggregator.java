...
import static no.nav.skanmotreferansenr.referansenr.PostboksReferansenrRoute.PROPERTY_FORSENDELSE_ZIPNAME;
...
public class PostboksReferansenrSkanningAggregator implements AggregationStrategy {
    ...
    public Exchange aggregate(Exchange oldExchange, Exchange newExchange) {
        try {
            if (oldExchange == null) {
                final PostboksReferansenrEnvelope envelope = new PostboksReferansenrEnvelope(newExchange.getProperty(PROPERTY_FORSENDELSE_ZIPNAME, String.class), FilenameUtils.getBaseName(newExchange.getIn().getHeader(Exchange.FILE_NAME, String.class)));
